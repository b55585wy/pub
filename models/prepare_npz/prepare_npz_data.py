...
def main():
    ...
    parser.add_argument("--data_dir", "-d", type=str, default="../sleep_data/sleep-cassette",
                        help="包含睡眠信息的EDF文件路径。")
    parser.add_argument("--output_dir", "-o", type=str, default="../sleep_data/sleepedf/prepared-cassette",
                        help="保存输出的目录。")
    ...
    for i, file in enumerate(psg_fnames):
        ...
        raw_ch_df.index = raw_ch_df.index / 100.0  # Scale the time index
        ...
        if len(raw_ch) % (EPOCH_SEC_SIZE * sampling_rate) != 0:
            # raise Exception("Something wrong")
            # Trim the data to be divisible by the epoch size
            excess_data_length = int(len(raw_ch) % (EPOCH_SEC_SIZE * sampling_rate))
            if excess_data_length != 0:
                # Trim the raw data and labels to match the closest lower multiple of epoch size
                raw_ch = raw_ch[:len(raw_ch) - excess_data_length]
                labels = labels[:len(raw_ch) // int(EPOCH_SEC_SIZE * sampling_rate)]
